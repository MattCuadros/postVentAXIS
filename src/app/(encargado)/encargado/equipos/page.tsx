...
export default function EquiposEncargadoPage() {
  ...
  if (!crews || !zones || !projects || !tickets) {
  ...
  return (
    <div className="pb-8">
      <PageHeader
        ...
        subtitle="Cuadrillas y subcontratos disponibles en tus zonas, organizados por obra."
      />

      {zones
        .filter((zone) => user?.zoneIds.includes(zone.id))
        .map((zone) => {
      ...
      {crews.map((crew) => {
        ...
        return (
          <li key={crew.id} className="rounded-lg border border-line-soft bg-surface p-4 shadow-card">
            ...
            <a
              href={"tel:" + crew.phone.replace(/\s/g, "")}
